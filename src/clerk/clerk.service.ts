...
export class ClerkService {
  ...
  async getUserEmailByUserId(userId: string): Promise<string | null> {
    try {
      ...
      if (primaryEmail) {
        this.logger.debug(`Found primary email for userId ${userId}: ${primaryEmail.emailAddress}`);
      ...
      if (user.emailAddresses.length > 0) {
        ...
        this.logger.debug(`Using fallback email for userId ${userId}: ${fallbackEmail}`);
